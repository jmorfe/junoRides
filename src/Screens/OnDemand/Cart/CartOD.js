...
function CartOD({ navigation, route }) {
      ...
      .then(res => {
        ...
        if (res && !isEmpty(res?.data)) {
          ...
          if (
            ...
            var delaySlot = res?.data?.products[0]?.delaySlot
              .replace(/,/g, '')
              .split(' ');
          ...
          setCartItems(
            !!res?.data && !!res?.data?.products ? res?.data?.products : [],
          );
  ...
  const _getAllOffers = (vendor, cartData) => {
    ...
    moveToNewScreen(navigationStrings.OFFERSONDEMAND, {
      vendor: cartData.products[0]?.vendor,
      cartId: cartData?.id,
    })();
  };
  ...
  const _paymentWithPlugnPayMethods = response => {
    ...
    let CardNumber = paramsData?.CardNumber
      ? paramsData?.CardNumber.split(' ').join('')
      : '';
    console.log('paramsData =>', paramsData);
    let expirydate;
    if (selectedPayment?.id == 50) {
      expirydate = paramsData?.year
        ? paramsData?.year.concat(paramsData?.date)
        : '';
    ...
    let savedCardId =
      (paramsData?.selectedSavedListCardNumber &&
        paramsData?.selectedSavedListCardNumber.id) ||
      '';
    ...
    if (selectedPayment?.id == 50 && !!CardNumber) {
  ...
  const placeOrder = () => {
    ...
    if (!!userData?.auth_token) {
      ...
      if (
        cartData?.without_category_kyc === 0 &&
        cartData?.category_kyc_count !== 0
      ) {
  ...
  const renderDates = () => {
    ...
    const my_dates = createDates(10)
    console.log("my_datesmy_dates", my_dates)
    ...
    return (
      ...
      >
        {!!my_dates ? my_dates.map((val, i) => {
          return (
            <TouchableOpacity
              onPress={() => onSelectDateFromCalendar(val)}
              style={{
                ...
                borderColor: selectedDateFromCalendar == val?.dateString ? themeColors?.primary_color : isDarkMode ? colors.whiteOpacity77 : colors.borderColor,
                backgroundColor: selectedDateFromCalendar == val?.dateString ? getColorCodeWithOpactiyNumber(themeColors?.primary_color.substring(1), 20) : 'transparent'
          ...
          {isEmpty(vendorAddress) && (
            <TouchableOpacity
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Image
                  source={imagePath.ic_right_arrow1}
                  resizeMode="contain"
  ...
  if (isShimmerLoading) {
    return (
      <WrapperContainer
        bgColor={isDarkMode ? MyDarkTheme.colors.background : colors.grey1}
        statusBarColor={colors.backgroundGrey}
        source={loaderOne}>
        <Header
          centerTitle={strings.ORDER_SUMMARY}
          // noLeftIcon
  ...
  const checkVendorSlots = async date => {
    if (businessType == 'laundry') {
      if (modalType !== 'pickup') {
        try {
          let apiData = `?vendor_id=${cartItems[0].vendor.id}&date=${date}&delivery=${dineInType}`
          console.log("apiDataapiData", apiData)
          const res = await actions.getVendorDropoffSlots(apiData,
            {},
            {
              code: appData?.profile?.code,
              timezone: RNLocalize.getTimeZone(),
  ...
  const onSelectTime = item => {
    ...
    actions
      .checkSlotsLimit(
        {
          schedule_datetime: selectedDateFromCalendar
            ? selectedDateFromCalendar
            : moment(new Date()).format('YYYY-MM-DD'),
          schedule_slot: item?.value,
          vendor_id: cartData?.products[0]?.vendor_id,
        },
        {
          code: appData?.profile?.code,
        },
  ...
  return (
    <WrapperContainer
      ...
      <Header
        centerTitle={strings.ORDER_SUMMARY}
        leftIcon={imagePath.icBackb}
        headerStyle={{ backgroundColor: isDarkMode ? MyDarkTheme.colors.background : colors.white }}
...
export default React.memo(CartOD);
